...
- **离线语音交互** — ESP-SR 框架集成，支持自定义唤醒词（"小白小白"）与 50+ 条命令词，无需云端
- **多模态 AI 推理** — 毫米波雷达 + MEMS 麦克风 + 环境特征的三模态融合，本地运行 INT8 量化模型 (~21KB)
...
| 模块 | 框架 | 功能 | 大小 |
|------|------|------|------|
| **唤醒词检测** | WakeNet (ESP-SR) | "小白小白" 唤醒 | ~100 KB |
| **命令词识别** | MultiNet (ESP-SR) | 50+ 条命令词 | ~200 KB |
| **音频前端** | AFE (ESP-SR) | AEC/BSS/NS | 内置 |
| **声音分类器** | TFLM | 场景声音识别 | ~12 KB |
| **雷达分析器** | TFLM | 运动特征提取 | ~4 KB |
| **融合决策器** | TFLM | 最终控制决策 | ~5 KB |
| **总计** | - | - | **~321 KB** |
...
| 命令类型 | 示例 | 效果 |
|----------|------|------|
| **场景切换** | "打开专注模式" | 5000K, 60% |
| **亮度调节** | "调亮一点" | 亮度 +10% |
| **色温调节** | "换成暖光" | 色温 2700K |
| **模式切换** | "打开演示模式" | 主灯 30%, 辅助灯亮 |
| **查询状态** | "现在是什么模式" | TTS 回复 |
| **取消操作** | "恢复自动" | 回到自动控制 |
