...
static wifi_state_t mock_state = WM_WIFI_STATE_DISCONNECTED;
...
{
	mock_state = WM_WIFI_STATE_GOT_IP;
	if (mock_cb) mock_cb(mock_state);
	return 0;
}
int wifi_manager_disconnect(void)
{
	mock_state = WM_WIFI_STATE_DISCONNECTED;
	if (mock_cb) mock_cb(mock_state);
	return 0;
}
wifi_state_t wifi_manager_get_state(void) { return mock_state; }
bool wifi_manager_is_connected(void) { return mock_state == WM_WIFI_STATE_GOT_IP; }
